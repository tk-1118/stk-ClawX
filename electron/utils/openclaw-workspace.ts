...
const CLAWX_BEGIN = '<!-- HNClaw:begin -->';
const CLAWX_END = '<!-- HNClaw:end -->';
...
/**
 * Merge a HNClaw context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeHNClawSection(existing: string, section: string): string {
...
/**
 * Detect and remove bootstrap .md files that contain only HNClaw markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairHNClawOnlyBootstrapFiles(): Promise<void> {
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed HNClaw-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove HNClaw-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge HNClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeHNClawContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('HNClaw context directory not found, skipping context merge');
    return 0;
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.HNClaw.md'));
  } catch {
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.HNClaw.md', '.md');
      ...
      const merged = mergeHNClawSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged HNClaw context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure HNClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureHNClawContext(): Promise<void> {
  let skipped = await mergeHNClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeHNClawContextOnce();
    if (skipped === 0) {
      logger.info(`HNClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`HNClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`HNClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
