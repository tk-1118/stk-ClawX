...
const CLAWX_BEGIN = '<!-- STKClaw:begin -->';
const CLAWX_END = '<!-- STKClaw:end -->';
...
/**
 * Merge a STKClaw context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeSTKClawSection(existing: string, section: string): string {
...
/**
 * Detect and remove bootstrap .md files that contain only STKClaw markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairSTKClawOnlyBootstrapFiles(): Promise<void> {
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed STKClaw-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove STKClaw-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge STKClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeSTKClawContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('STKClaw context directory not found, skipping context merge');
    return 0;
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.STKClaw.md'));
  } catch {
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.STKClaw.md', '.md');
      ...
      const merged = mergeSTKClawSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged STKClaw context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure STKClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureSTKClawContext(): Promise<void> {
  let skipped = await mergeSTKClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeSTKClawContextOnce();
    if (skipped === 0) {
      logger.info(`STKClaw context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`STKClaw context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`STKClaw context merge: ${skipped} file(s) still missing after ${MAX_RETRIES} retries`);
}
